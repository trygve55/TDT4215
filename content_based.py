...
def bernoulli_bayes(df, k=20):
    ...
    for idx, user in enumerate(tqdm(users)):
        ...
        df_unseen = df_unique_docs[
            ~df_unique_docs["tid"].isin(user_df["tid"])]  # The observations of unique articles the user hasn't seen
        zero_class_sample = df_unseen.sample(
            n=user_df.shape[0])  # A random sample of these observations (as many as articles the user has seen).
        ...
        user_df_test = pd.concat([df_unseen, zero_class_train_sample]).drop_duplicates(
            keep=False)  # Drops observations seen in both df_unseen and zero_class_train_sample
